 ...
 * @param {*} showTabBar
 */
export const getWindowHeight = (showTabBar = true,height=0) => {
  ...
  if (process.env.TARO_ENV === 'rn') {
    return windowHeight - statusBarHeight - NAVIGATOR_HEIGHT - tabBarHeight-height
  }

  if (process.env.TARO_ENV === 'h5') {
    return `${windowHeight - tabBarHeight-height}px`
  }
  ...
  if (process.env.TARO_ENV === 'alipay') {
    ...
    return `${windowHeight - statusBarHeight-height  + (showTabBar ? 0 : TAB_BAR_HEIGHT)}px`
  }
